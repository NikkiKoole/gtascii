...
function love.mousereleased(x,y)
   ...
   if (x > margin and x < margin + smallSize) then
	 ...
	 love.math.setRandomSeed(seed)
	 perlin:load()
	 worldZoomed = createZoomedinWorld(worldWidth, worldHeight, 8*2, worldViewRect.x*2, worldViewRect.y*2, cities)
	 zoomedCanvas = generateOverViewCanvas(worldZoomed)
	 worldZoomedComplete =  createZoomedinWorld(worldWidth, worldHeight, 8*8*2, worldViewRect.x * 8*2 + worldCompleteRect.x , worldViewRect.y * 8*2 + worldCompleteRect.y, cities )
	 worldCompleteCanvas = generateOverViewCanvas(worldZoomedComplete)
